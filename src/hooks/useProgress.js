...
export function useProgress(profileId) {
  ...
  const recordAttempt = useCallback((cardData, cardStateSnapshot, timerSnapshot) => {
    ...
    const attempt = {
      ...
      conceptsRevealed:          cardStateSnapshot.conceptsRevealed || [],
      missingConstraintsRevealed: cardStateSnapshot.missingConstraintsRevealed || false,
      notesWritten:              (cardStateSnapshot.notes || '').length > 0,
      notesLength:               (cardStateSnapshot.notes || '').length,
      modificationViewed:        cardStateSnapshot.modificationViewed || false,
      cardCompleted:             true,
